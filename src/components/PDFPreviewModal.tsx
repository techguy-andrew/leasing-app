...
export default function PDFPreviewModal({
  ...
  const [resolvedUnit, setResolvedUnit] = useState<string | null>(null)
  ...
  const handleFieldChange = (field: keyof Omit<ExtractedData, 'confidence' | 'metadata'>, value: string) => {
    ...
    // Clear resolved unit and errors when user edits property/unitNumber
    if (field === 'property' || field === 'unitNumber') {
  ...
  const handleUseData = async () => {
    ...
    // If property and unitNumber are both provided, resolve to unitId first
    if (editedData.property?.trim() && editedData.unitNumber?.trim()) {
      setIsResolving(true)
      setResolveError(null)

      try {
        const response = await fetch('/api/units/find-or-create', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            propertyName: editedData.property.trim(),
            unitNumber: editedData.unitNumber.trim()
          })
        })
        ...
        if (!response.ok) {
          ...
          setResolveError(result.error || 'Failed to find or create unit')
          setIsResolving(false)
          return
        }

        // Success! Add unitId to the data
        const dataWithUnit: ExtractedData = {
          ...editedData,
          unitId: result.unitId.toString()
        }

        setResolvedUnit(result.message)
        setIsResolving(false)

        // Pass data with unitId to parent
        onUseData(dataWithUnit)
      } catch (error) {
      ...
      // No property/unit to resolve, just pass the data as-is
          ...
          >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-blue-700 mt-1">
                        ...
                        {editedData.property && editedData.unitNumber && (
                          <> The unit will be automatically created or found based on the property and unit number.</>
                        )}
                      </p>
                    </div>
                  </div>
                ...
                {resolvedUnit && (
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg mt-4">
                    <div className="flex gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-green-900">Unit Resolved</p>
                        <p className="text-xs text-green-700 mt-1">{resolvedUnit}</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 flex-shrink-0">
              ...
              >
                ...
                {isResolving ? 'Resolving Unit...' : 'Use This Data'}
              </button>
            </div>
          </motion.div>
        </div>
